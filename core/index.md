# 通用文档索引

| 文件 | 说明 |
| --- | --- |
| `px4_code_structure.md` | PX4 代码结构与主要模块入口。 |
| `px4_nuttx_architecture.md` | PX4 与 NuttX 架构分层。 |
| `obstacle_avoidance.md` | 避障机制与关键参数。 |
| `estimator_status_control_flags.md` | EKF 控制标志位与 bitmask 解释。 |
| `px4_parameter_generation.md` | 参数生成链路与板级差异。 |
| `make_px4_sitl_pipeline.md` | 解析 `make px4_sitl <model>` 的执行流程。 |
| `px4_position_topics.md` | 位置相关 uORB 话题对比。 |
| `mavros_control_interfaces.md` | MAVROS 接口与控制路径。 |
| `geofence_3x3x3_lab.md` | 3x3x3 场景地理围栏实验记录。 |

> 新增文档时，请在此表格中追加条目，便于快速定位。 
