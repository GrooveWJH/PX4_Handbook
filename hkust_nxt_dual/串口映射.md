...
> 本文以 `archive/board_config/hkust#2`（2025-12-05）为示例，说明如何从 `.px4board`、`defconfig`、`board.h` 推导接口关系。将来若新增快照，只需在 `archive/` 中查看对应版本，本文无需修改。
...
| TELEM2 | `/dev/ttyS3` | UART4：TX=PB9，RX=PB8 | `CONFIG_STM32H7_UART4=y`，`CONFIG_UART4_BAUD=921600`，带 RX/TX DMA | 默认用于串行测距仪（`SENS_TFMINI_CFG=102`，波特率 115200），如需其它用途先禁用对应驱动 |
...
| USB-C CDC | `/dev/ttyACM0` | STM32 OTG FS + CDC ACM (`CONFIG_CDCACM_*`) | `CONFIG_CDCACM_CONSOLE=y`，`CONFIG_NSH_USBCONSOLE=y` | 同时承担 Bootloader、NSH Console；MAVLink 默认关闭 |

> `/dev/ttyS*` ↔ MCU 外设的对应关系可在旧版 `old_defconfig.txt` 的注释（`#ttyS0` 等）以及 PX4 STM32 驱动里找到；表格展示了这一映射在快照 #2 下的实例。新的快照若调整串口角色，只需按照同样的方法推导即可。

> `Nuttx defconfig` 中启用了 `CONFIG_NSH_USBCONSOLE`+`CONFIG_CDCACM_CONSOLE`，因此 NSH 始终从 USB CDC (`/dev/ttyACM0`) 输出来，这与各 `SER_TEL*` 参数无关。
### 如何调整/验证
